...
class PCA9685:
    ...
    def __init__(self, address=0x40, busId=1, debug=False):
    ...
    def init(self):
        config = 0
        config |= self.__PCA9685_MODE1_RESTART_ENABLED
        ...
        config |= self.__PCA9685_MODE1_ALLCALL_ENABLED
...
#        print 'r:' + r + ',g: ' + g + ',b: ' + b
    ...
    def enMotor(self, enable):
        if enable:
            self.setPwmDuty(12, 1)
        else:
            self.setPwmDuty(12, 0)

    def motorSpeed(self, lr, speed):
        if lr == 0:
            if speed > 0:
                self.setPwmDuty(11, 0)
                self.setPwmDuty(10, 1)
                self.setPwmDuty( 9, speed)
            else:
                self.setPwmDuty(11, 1)
                self.setPwmDuty(10, 0)
                self.setPwmDuty( 9, -speed)
        elif lr == 1:
            if speed > 0:
                self.setPwmDuty(13, 0)
                self.setPwmDuty(14, 1)
                self.setPwmDuty(15, speed)
            else:
                self.setPwmDuty(13, 1)
                self.setPwmDuty(14, 0)
                self.setPwmDuty(15, -speed)
            



#pwm = PCA9685(0x40, 1, True)
#pwm.setPwmClock(1000)
#pwm.setAllPwm(0, 4095)
#pwm.setPwmDuty(0, 0.0)
#pwm.setPwmDuty(1, 0.5)
#pwm.setPwmDuty(2, 0.0)
